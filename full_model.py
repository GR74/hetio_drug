...
dis_txt  = torch.tensor(FEAT["disease_txt"], dtype=torch.float32)
...
def train(model, E, pos_by_rel, comp_pool, seen_dis=None, epochs=200, lr=0.01):
...
print(f"      text feature    AUROC={MD('auc_txt')[0]:.3f}+-{MD('auc_txt')[1]:.2f}   (name embedding transfers)")
...
try:
    ...
    ax[1].set_xticks([0, 1]); ax[1].set_xticklabels(["id-embed", "text feature"])
    ax[1].set_title("Unseen DISEASES (AUROC)")
    fig.suptitle("Inductive zero-shot on BOTH sides of the graph (full Hetionet)",
                 fontsize=10, weight="bold")
    fig.tight_layout(); fig.savefig(HERE / "full_result.png", dpi=140)
    print("\nSaved figure -> full_result.png")
...
json.dump({
    ...
    "inductive_diseases": {"id_auroc": round(MD("auc_id")[0], 3), "text_auroc": round(MD("auc_txt")[0], 3)},
    "explainer_example": {"drug": cname, "disease": dname, "score": round(base, 3),
                          "top_proteins": [gene_name.get(g, "?") for _, g in top]},
}, open(HERE / "full_results.json", "w"), indent=2)
